<tr class="bg-white lg:hover:bg-gray-100 flex lg:table-row flex-row lg:flex-row flex-wrap lg:flex-no-wrap mb-10 lg:mb-0">
    ...
    <td class="w-full lg:w-auto p-3 text-gray-800 text-center border border-b text-center block lg:table-cell relative lg:static">
        <span class="lg:hidden absolute top-0 left-0 bg-blue-200 px-2 py-1 text-xs font-bold uppercase">Etiquetas</span>
        @foreach ($allTags as $tag)
            {{-- <span class="rounded-full py-1 px-1..." style="background-color: #{{$tag->color}}">
                ...
                <span class="ml-2 font-medium">{{$tag->nombre}}&nbsp;&nbsp;&nbsp;</span>
                </span> --}}
                {{-- <span class="rounded bg-green-400 py-1 px-3 text-xs font-bold" style="background-color: {{$tag->color}}">{{$tag->nombre}}</span> --}}
            ...
            <div class="col-span-1 sm:col-span-2 md:col-span-1 lg:col-span-1 xl:col-span-1...">
                @if($update == True)
                    ...
                    @php
                        // dd($teams);
                    @endphp
                    <x-editar-device-modal id="{{$id}}" nombre="{{$nombre}}" :allTags="$allTags"  :teams="$teams" :users="$users" :categorias="$categorias" />
               @endif
            </div>
            <div class="col-span-1 sm:col-span-2 md:col-span-1 lg:col-span-1 xl:col-span-1...">
                @if($delete == True)
                    <form action="{{ route('Devices.destroy',$id)}}" method="POST" accept-charset="utf-8" style="width: auto">
                        @method('DELETE')
                        @csrf
                        <button type="submit">
