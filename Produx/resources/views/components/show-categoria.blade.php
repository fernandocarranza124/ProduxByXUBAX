            ...
            <div class="col-span-1 sm:col-span-2 md:col-span-1 lg:col-span-1 xl:col-span-1...">
                @if($update == True)
                    ...
                    @php
                        // dd($teams);
                    @endphp
                    <x-editar-categoria-modal id="{{$id}}" nombre="{{$nombre}}" ownerId={{$owner_id}} :teams="$teams" :users="$users" />
               @endif
            </div>
            <div class="col-span-1 sm:col-span-2 md:col-span-1 lg:col-span-1 xl:col-span-1...">
                @if($delete == True)
                    <form action="{{ route('Categorias.destroy',$id)}}" method="POST" accept-charset="utf-8" style="width: auto">
                        @method('DELETE')
                        @csrf
                        <button type="submit"><svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" viewBox="0 0 20 20" fill="currentColor">
